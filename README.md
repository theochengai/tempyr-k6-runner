...
PerfTest's authenticated completion callback and D1/R2 state remain authoritative for Run status and persisted reports.

The generated PerfTest k6 script emits frozen authored-request tags such as:
...
These tags preserve the immutable authored API identity frozen into each Run and are available to the native PerfTest result pipeline without depending on resolved runtime URLs.
